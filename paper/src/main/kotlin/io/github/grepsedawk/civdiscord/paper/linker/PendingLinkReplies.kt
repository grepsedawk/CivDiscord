...
/**
 * In-memory map of request-id -> mc-uuid of the player who triggered the LinkRequest, so
 * when the LinkReply lands we know which player to whisper the code (or error) to.
 *
 * Entries expire after [ttlMillis] so a dropped LinkReply (Velocity restart, network blip,
 * core bus drop) can't leak memory.
 */
