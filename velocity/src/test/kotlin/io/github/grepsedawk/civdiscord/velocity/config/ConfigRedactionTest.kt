...
class ConfigRedactionTest {
    ...
    fun `Config toString does not leak nested secrets`() {
        val cfg = Config(
            ...
            namelayerDb = NameLayerDbConfig(
                host = "db.example.com",
                port = 3306,
                user = "nl",
                password = "namelayer-db-secret-password-xyz",
                database = "citadel",
            ),
        ...
        cfg.toString() shouldNotContain "super-secret-token-value-12345"
        cfg.toString() shouldNotContain "patreon-secret-access-token-abcdef"
        cfg.toString() shouldNotContain "namelayer-db-secret-password-xyz"
    }

    @Test
    fun `NameLayerDbConfig toString redacts password`() {
        val cfg = NameLayerDbConfig(
            host = "db.example.com",
            port = 3306,
            user = "nl",
            password = "namelayer-db-secret-password-xyz",
        ...
        cfg.toString() shouldNotContain "namelayer-db-secret-password-xyz"
        cfg.toString() shouldContain "host=db.example.com"
    }
}
