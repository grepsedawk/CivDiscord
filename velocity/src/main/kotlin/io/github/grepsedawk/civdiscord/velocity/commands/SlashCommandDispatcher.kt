...
class SlashCommandDispatcher(
    ...
    private val stats: StatsCommand,
    private val status: StatusCommand,
    ...
    override fun onSlashCommandInteraction(event: SlashCommandInteractionEvent) {
        when (event.name) {
            ...
            "status" -> if (requireHomeGuild(event)) status.handle(event)
            "stats" -> if (requireHomeGuild(event)) stats.handle(event)
    ...
    override fun onCommandAutoCompleteInteraction(event: CommandAutoCompleteInteractionEvent) {
        ...
        if (event.name == "relay" && event.focusedOption.name == "namelayer-group") {
            val matches = relayGroupsForChannel(event.channel.idLong)
                .filter { it.startsWith(event.focusedOption.value, ignoreCase = true) }
                .take(25)
            event.replyChoiceStrings(matches).queue(null, restErrorHandler)
        }
