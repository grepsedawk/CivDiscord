...
constructor(
    ...
    fun onInit(event: ProxyInitializeEvent) {
        ...
        // Proxy-side /discord intercepts before any Paper backend sees the command, so this is
        // the authoritative path for linked players regardless of which backend they're on.
        // Paper's DiscordCommand stays in place as a fallback for now.
        val discordPlayerCmd = DiscordPlayerCommand(linkTokens, bindings, logger)
        val discordCmdMeta = server.commandManager.metaBuilder("discord").plugin(this).build()
        server.commandManager.register(discordCmdMeta, discordPlayerCmd)
        ...
        val replyRegistry = ReplyRegistry()
        ...
        val sendSnitchToDiscord = { channelId: Long, text: String, ping: io.github.grepsedawk.civdiscord.core.relay.SnitchPing? ->
            jda?.getTextChannelById(channelId)?.let { ch ->
                val action = ch.sendMessage(text)
                when (ping) {
                    is io.github.grepsedawk.civdiscord.core.relay.SnitchPing.Role -> action.mentionRoles(ping.id.toString())
                    is io.github.grepsedawk.civdiscord.core.relay.SnitchPing.User -> action.mentionUsers(ping.id.toString())
                    is io.github.grepsedawk.civdiscord.core.relay.SnitchPing.Everyone ->
                        action.setAllowedMentions(java.util.EnumSet.of(net.dv8tion.jda.api.entities.Message.MentionType.EVERYONE))
                    null -> action.setAllowedMentions(emptyList<net.dv8tion.jda.api.entities.Message.MentionType>())
                }
                action.queue(null, restErrorHandler)
            }
            Unit
        }
        val chatLimiter =
        ...
        val chatRelay =
            ChatRelay(
                relays = relays,
                bindings = bindings,
                permService = nameLayerPermService,
                ...
                unbind = { ch, group -> relaySvc.unbind(ch, group) },
                rateLimiter = chatLimiter,
                maxTextLength = config.chat.maxLength,
            )

        val writerlessNotifier =
        ...
        val listeners =
            listOf(
                ...
                MessageRelayListener(
                    ...
                    permService = nameLayerPermService,
                    unbind = { ch, group -> relaySvc.unbind(ch, group) },
                    writerlessNotifier = writerlessNotifier,
                    worker = relayWorker,
                ),
                dispatcher,
            )
        ...
        val snitchRelay = SnitchRelay(
            relays = relays,
            bindings = bindings,
            permService = nameLayerPermService,
            sendToDiscord = sendSnitchToDiscord,
            logger = logger,
        )
        bridge = BridgeServer(
            ...
            handlersFactory = { b ->
                io.github.grepsedawk.civdiscord.velocity.bridge.ServerInboundHandlers(
                    ...
                    onChatToDiscord = { event -> relayWorker.execute { chatRelay.dispatch(event) } },
                    onNameLayerReply = { reply -> replyRegistry.complete(reply) },
                    onStatusRequest = { req ->
                        val binding =
