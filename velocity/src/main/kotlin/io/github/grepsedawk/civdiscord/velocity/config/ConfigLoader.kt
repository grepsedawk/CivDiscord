...
object ConfigLoader {
    ...
    fun load(dataDir: File, env: (String) -> String? = System::getenv): Config {
        ...
        val nldb =
            (raw["namelayer_db"] as? Map<*, *>)?.let { nm ->
                val pwFromYaml = (nm["password"] as? String)?.let { expand(it, env) }
                val pwFromEnv = env("CIVDISCORD_NAMELAYER_DB_PASSWORD")?.takeIf { it.isNotBlank() }
                val password = pwFromEnv
                    ?: pwFromYaml
                    ?: error("config.yml: namelayer_db.password is required (or set CIVDISCORD_NAMELAYER_DB_PASSWORD)")
                NameLayerDbConfig(
                    host = (nm["host"] as? String) ?: error("config.yml: namelayer_db.host is required"),
                    port = (nm["port"] as? Number)?.toInt() ?: 3306,
                    user = (nm["user"] as? String) ?: error("config.yml: namelayer_db.user is required"),
                    password = password,
                    database = (nm["database"] as? String) ?: error("config.yml: namelayer_db.database is required"),
                )
            }
        ...
        val chat =
            (raw["chat"] as? Map<*, *>)?.let { cm ->
                val defaults = ChatConfig()
                ChatConfig(
                    maxLength = (cm["max_length"] as? Number)?.toInt() ?: defaults.maxLength,
                    burst = (cm["burst"] as? Number)?.toInt() ?: defaults.burst,
                    refillPerSecond = (cm["refill_per_second"] as? Number)?.toDouble() ?: defaults.refillPerSecond,
                )
            } ?: ChatConfig()
    ...
    private fun expand(value: String, env: (String) -> String?): String = ENV_PATTERN.replace(value) { match -> env(match.groupValues[1]) ?: match.value }
}
