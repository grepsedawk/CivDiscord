        ...
        jda.updateCommands().addCommands(
            ...
            Commands.slash("relay", "Bind Discord channels to NameLayer chat groups")
                ...
                .addSubcommands(
                    ...
                    SubcommandData("unbind", "Remove a binding from this channel")
                        .addOption(OptionType.STRING, "namelayer-group", "Group to unbind (required if channel has multiple)", false, true),
                    SubcommandData("writer", "Set which bound group is the writer (2-way)")
                        .addOption(OptionType.STRING, "namelayer-group", "Group to promote to writer", true, true),
                    SubcommandData("list", "List all relays in this guild"),
                    SubcommandData("show", "Show this channel's bindings and settings"),
                    SubcommandData("set", "Update a property of a binding on this channel")
                        .addOption(OptionType.STRING, "property", "show-snitches | chat-format | snitch-ping", true)
                        .addOption(OptionType.STRING, "value", "new value (for snitch-ping: @role/@user, or 'null' to clear)", true)
                        .addOption(OptionType.STRING, "namelayer-group", "Group to set (required if channel has multiple)", false, true),
                ),
