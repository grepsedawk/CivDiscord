...
Edit `dev/velocity/plugins/civdiscord/config.yml` and `dev/paper-citadel/plugins/civdiscord/config.yml` with your test token + server name (`citadel`).
...
- **Reset format**: `/relay set property:chat-format value:null` → `Set chat-format=\`(default)\`.`
