...
class BindingDao(private val db: Database) {
    ...
    private fun isSqliteUniqueViolation(e: ExposedSQLException): Boolean {
        var cur: Throwable? = e
        while (cur != null) {
            if (cur is SQLException && cur.errorCode == SQLITE_CONSTRAINT_UNIQUE) return true
            cur = cur.cause
        }
        return false
    }

    fun findByDiscordId(discordId: Long): Binding? =
        transaction(db) {
            BindingsTable
                .selectAll()
                .where { BindingsTable.discordId eq discordId }
    ...
    fun findByMcUuid(mcUuid: UUID): Binding? =
        transaction(db) {
            BindingsTable
                .selectAll()
                .where { BindingsTable.mcUuid eq mcUuid.toString() }
    ...
    fun delete(discordId: Long): Boolean =
        transaction(db) {
            BindingsTable.deleteWhere { BindingsTable.discordId eq discordId } > 0
        }

    private fun toBinding(row: ResultRow) =
        Binding(
