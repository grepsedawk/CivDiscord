...
class RelayService(private val dao: RelayDao) {
    ...
    fun bind(guildId: Long, channelId: Long, group: String, createdBy: Long): BindResult = when (dao.bind(guildId, channelId, group, createdBy)) {
        is RelayDao.BindOutcome.Inserted -> BindResult.Bound
        RelayDao.BindOutcome.AlreadyBound -> BindResult.ChannelAlreadyBound
    }

    fun unbind(channelId: Long): UnbindResult = if (dao.unbind(channelId)) UnbindResult.Unbound else UnbindResult.NotBound
    ...
    fun setShowSnitches(channelId: Long, value: Boolean): SetResult = if (dao.setShowSnitches(channelId, value) > 0) SetResult.Updated else SetResult.NotBound

    fun setChatFormat(channelId: Long, value: String?): SetResult = if (dao.setChatFormat(channelId, value) > 0) SetResult.Updated else SetResult.NotBound
}
