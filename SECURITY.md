...
Velocity proxy and Paper backends communicate over the `civdiscord:bridge` plugin-message channel. By default this is **unauthenticated**: any plugin on either side that knows the channel name can read or inject payloads. HMAC signing is supported but **opt-in** — drop a `secret.key` file in both `plugins/civdiscord/` directories to enable.
...
`libs/` ships CivMC plugin primitives. Each jar has a pinned SHA-256 in `libs/CHECKSUMS.sha256`; CI verifies on every build. PR reviewers must verify any change to those files. Replacing a vendored jar without updating its checksum will fail CI.
