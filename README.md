...
namelayer_db:                  # the backend's NameLayer MariaDB — proxy reads it
  ...
  database: citadel
# patreon block optional — see the commented template in the default config.yml.
```
...
- **Discord channel IDs** Kira was bridging to. Point CivDiscord's `/relay bind` at the same channels.
...
5. **Run post-cutover admin commands:**
   ...
   - `/relay bind <group>` in each channel that used to be a Kira relay room — CivDiscord doesn't read Kira's routes. (The binder must hold `READ_CHAT` in that NameLayer group.)
...
| `/discord link`, `/discord status` | all players | the Velocity proxy or any backend |
...
- **Link codes**: 12-character codes drawn from a CSPRNG alphabet, single-use, expire after ~10 minutes. Minting is unthrottled, but `/link <code>` redemption is rate-limited per Discord user. Each redemption is audit-logged. Codes are *not* secret long-term — they're one-shot — but the link flow prints them in plain in-game chat (see streamer note below).
...
See also [`CONTRIBUTING.md`](CONTRIBUTING.md) and [`SECURITY.md`](SECURITY.md). Release notes live on the [GitHub releases page](https://github.com/grepsedawk/CivDiscord/releases).
